...
public static class AdvancedFluentLayoutExtensions
{
    ...
    public static FluentLayout? GetLayoutById(this IEnumerable<FluentLayout> layouts, string identifier) =>
        layouts.FirstOrDefault(x => x.Identifier?.Equals(identifier) is true);
    ...
    public static IEnumerable<FluentLayout> AdvancedVerticalStackPanelConstraints(this UIView parentView,
                                                                                  ...
                                                                                  params UIView[] views)
    {
        string? previousIdentifierPrefix = default;
        margins ??= new Margins();
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            var childLeftMargin = childrenLeftMargins?[i] ?? default;
            ...
            var childRightMargin = childrenRightMargins?[i] ?? default;
            ...
            var childTopMargin = childrenTopMargins?[i] ?? default;
            layouts.Add(i == 0
                ? view.Top()
                    .EqualTo()
                    .TopOf(parentView)

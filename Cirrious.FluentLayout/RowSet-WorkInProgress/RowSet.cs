...
public class RowTemplate : IRowTemplate
{
    ...
    {
        WeightedWidthColumn? firstWeightedColumn = default;
        UIView? firstWeightedView = default;
        ...
        for (var i = 0; i < views.Length; i++)
        {
            ...
            if (column is WeightedWidthColumn weightedColumn)
            {
            ...
            if (column is FixedWidthColumn fixedColumn)
            {
    ...
    {
        if (_columnDefinitions.TryGetValue(index, out var column))
            return column;
